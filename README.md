...
## Модель процессора
Модель Half68k выполнена на Python в файле machine.py. Она имитирует работу процессора с точностью до такта (tick), использует микропрограммируемое управление (mc) и поддерживает суперскалярное выполнение инструкций (superscalar).
...
### Ввод-вывод
Используется потоковый ввод-вывод (`stream`) через порты, отображённые на память (`memory-mapped`):
- `IN_PORT = 0xFFFFFF00`
- `OUT_PORT = 0xFFFFFF04`

При старте модели входной текстовый файл загружается в буфер. При чтении из `IN_PORT` модель возвращает очередной байт; при исчерпании возвращается 0. Запись в `OUT_PORT` добавляет байт в выходной буфер, который выводится после останова.

### Стек и подпрограммы
Стек растёт вниз; `SP` инициализируется значением `DATA_MEM_SIZE * 4 - 4`. При выполнении `jsr` адрес возврата сохраняется на стеке; `rts` восстанавливает `PC` и корректирует `SP`. Локальные переменные могут размещаться через `-(SP)` / `(SP)+`.
...
Файл `tools/test_translator.py` проверяет работу препроцессора и парсера на уровне отдельных инструкций и режимов адресации.

### Модульные тесты процессора
Файл `tools/test_machine.py` тестирует выполнение основных инструкций (`mv`, `add`, `cmp`, переходы, `jsr/rts`) без запуска полноценного моделирования. Проверяются конечные состояния регистров и флагов.

### Golden-тесты
Golden-тесты (`tools/test_golden.py`) проверяют полную цепочку «транслятор → модель» на семи программах:
- `double_precision` – 64-битное сложение с демонстрацией переноса
- `cat` – эхо-повтор входных данных
- `features` – макросы, условная компиляция, предекрементная адресация
- `hello` – вывод Hello, World!
- `hello_user_name` – запрос имени и вывод приветствия
- `prob1` – Euler problem 4 (поиск наибольшего палиндрома-произведения)
- `sort` – сортировка пузырьком массива из 5 чисел

Для каждого теста в папке `golden/` хранится YAML-файл с эталонными данными:
```
in_source: |
  .data
  ...
